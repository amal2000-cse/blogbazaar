...
export const updateUser = async (req, res, next) => {
  ...
  if(req.body.username){
    ...
    if(req.body.username.includes(' ')){
      return next(
        errorHandler(400, "Username cannot contain spaces")
      );
    }
    ...
    try {
      const updatedUser = await User.findByIdAndUpdate(req.params.userId, {
        ...
        //now with this, it is going to send back the new information
        //(new: true)
      }, {new :true})

      //and we will also be removing the password from the user data
